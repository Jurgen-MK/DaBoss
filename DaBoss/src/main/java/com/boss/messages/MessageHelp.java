...
public class MessageHelp extends MessageListener implements EventListener<MessageCreateEvent> {
	...
	public Mono<Void> processCommand(Message eventMessage) {
		return Mono.just(eventMessage).filter(message -> message.getAuthor().map(user -> !user.isBot()).orElse(false))
				.filter(message -> message.getContent().equalsIgnoreCase("Босс, хелп")).flatMap(Message::getChannel)
				.flatMap(channel -> channel.createMessage(createResponse())).then();
	}

	public EmbedCreateSpec createResponse() {
		EmbedCreateSpec embed = EmbedCreateSpec.builder().color(Color.GREEN).title("Помащ Босса")
